...
class TestDepositPositiveUI(BaseUITest):
    ...
    def test_deposit_positive(
        ...
        # ШАГ 3: депозит
        dashboard.open_deposit().deposit_money(str(account.id),deposit_amount)
        # Проверяем аллерт
        dashboard.check_alert_message_and_accept(BankAlert.DEPOSIT_MONEY_ACCOUNT)

        # ШАГ 4: проверка через API
        transactions = api_manager.user_steps.get_account_transactions(user_request,account.id)
