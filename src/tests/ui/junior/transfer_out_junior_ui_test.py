...
class TestTransferOutAgainUI(BaseUITest):
    @pytest.mark.userfixtures("admin_user_request", "api_manager", "user_request")
    def test_transfer_out_again(self, page: Page, admin_user_request, deposit_amount, transfer_amount,api_manager,user_request: CreateUserRequest):
        ...
        page.locator("li", has_text="TRANSFER_OUT").get_by_role("button", name="Repeat").click()
        page.locator("select.form-control").select_option(user_accounts[0].accountNumber.replace("ACC", "")) # BUG:UI подставляет одинаковый account (source == target),
        #хотя должен быть source=5916, target=5917
        page.locator("#confirmCheck").check()
        page.get_by_role("button", name="Send Transfer").click()
        ...
        def handle_repeat_transfer_dialog(dialog: Dialog):
            expected = (
                ...
                f"from Account {user_accounts[0].accountNumber} "
                f"to {user_accounts[1].accountNumber}"
            )
            assert expected in dialog.message
            dialog.accept()
