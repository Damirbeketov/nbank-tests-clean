...
class TestTransferPositiveUI(BaseUITest):
    ...
    def test_user_can_create_account(self, page: Page, admin_user_request, deposit_amount, transfer_amount,api_manager,user_request: CreateUserRequest):
        ...
        def handle_create_transfer_dialog(dialog: Dialog):
            expected = f"❌ Please fill all fields and confirm."
            assert expected in dialog.message
            dialog.accept()
