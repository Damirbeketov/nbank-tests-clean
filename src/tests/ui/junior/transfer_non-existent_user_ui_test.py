...
class TestTransferPositiveUI(BaseUITest):
    ...
    def test_user_can_create_account(self, page: Page, admin_user_request, deposit_amount, invalid_account, transfer_amount,api_manager,user_request: CreateUserRequest):
        ...
        page.get_by_placeholder("Enter recipient account number").fill(invalid_account)
        ...
        def handle_create_transfer_dialog(dialog: Dialog):
            expected = f"❌ No user found with this account number."
            assert expected in dialog.message
            dialog.accept()
