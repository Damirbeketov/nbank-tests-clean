...
class TestProfileRenameNegativeUI(BaseUITest):
    ...
    def test_user_can_create_account(self, page: Page, admin_user_request, edit_profile, deposit_amount, transfer_amount, api_manager,
        ...
        name = page.locator("h2.welcome-text span")
        expect(name).to_have_text("noname")

        # создаем имя пользователю
        page.locator("div.profile-header").click()
        page.get_by_placeholder("Enter new name").fill(edit_profile)

        # проверяем аллерт создания имени пользователя
        def handle_create_name_dialog(dialog: Dialog):
            expected = f"✅ Name updated successfully!"
            assert expected in dialog.message
            dialog.accept()

        page.once("dialog", handle_create_name_dialog)
        page.get_by_role("button", name="💾 Save Changes").click()
        page.get_by_role("button", name="🏠 Home").click()

        # проверяем что у пользователя появилось новое имя
        name = page.locator("h2.welcome-text span")
        expect(name).to_have_text(edit_profile)

        # проверка изменения имени через API
        rename = api_manager.user_steps.get_customer_profile(new_user_request)
        assert rename.name == edit_profile
        ...
        page.select_option(".account-selector", value=user_accounts[0].accountNumber.replace("ACC", ""))
        page.get_by_placeholder("Enter recipient name").fill("")
        ...
        # ШАГ 13: Проверяем Алерт транфсфера
        def handle_create_transfer_dialog(dialog: Dialog):
            expected = f"❌ The recipient name does not match the registered name."
            assert expected in dialog.message
            dialog.accept()
