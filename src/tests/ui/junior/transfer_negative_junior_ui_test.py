...
class TestTransferPositiveUI(BaseUITest):
    ...
    def test_user_can_create_account(self, page: Page, admin_user_request, deposit_amount, invalid_amount,api_manager,user_request: CreateUserRequest):
        ...
        page.get_by_placeholder("Enter amount").fill(str(invalid_amount))
        page.locator("#confirmCheck").check()

        # ШАГ 13: Проверяем Алерт транфсфера
        def handle_create_transfer_dialog(dialog: Dialog):
            expected = f"❌ Error: Invalid transfer: insufficient funds or invalid accounts"
            assert expected in dialog.message
            dialog.accept()
